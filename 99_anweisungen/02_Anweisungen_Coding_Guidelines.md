...
5. **Code-Index & Suche (Token-Disziplin):** Vor und nach jeder Arbeit `python generate_index.py` ausführen. Zur Lokalisierung von Endpunkten, Tabellen, Composables oder Funktionen **immer zuerst `.agent_index.json`** lesen. Dank der indexierten 1-basierten Zeilennummern können Code-Ausschnitte direkt gezielt geöffnet werden. Grep/Repo-Suche ist erst als Fallback erlaubt, falls das Element nicht im Index steht.
