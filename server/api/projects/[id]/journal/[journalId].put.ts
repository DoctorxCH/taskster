...
export default defineEventHandler(async (event) => {
  ...
  db.prepare(`
    ...
    SET title = ?, content = ?, category = ?, visibility = ?, allowed_group_id = ?, metadata = ?, updated_at = datetime('now')
    WHERE id = ? AND project_id = ?
  `).run(title, content, category, visibility, allowedGroupId, metaJson, journalId, projectId)
